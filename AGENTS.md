...
**Surfaces carry three roles, and a region takes one by what it *is*, not by which component draws it**: `base-100` is the **page** (toolbar, pane content, diagram canvas), `base-200` is a **band** (section headers — the two picker accordion headers, the editor tab row, `RenderingStrip`), `base-300` is an **edge** (dividers). Bands are the only raised chrome; everything else is one surface, so a header reads as a header without a border. Three utilities in `index.css` carry the rest, and each documents its own numbers: **`section-header`** for a band's title, which must sit on the element holding the text since daisyUI's `.btn` and `.tab` set their own font-size; **`option`** and **`option-selected`** for anything selectable, which draw a selection in value rather than hue. Hue is reserved for `Copy link` and the diagram itself — the phone's Edit/View toggle is the one exception. Chrome elevates *toward gray* — lighter in dark, darker in light — leaving the page at the extreme in both, which is why `index.css` inverts daisyUI's dark ramp rather than mapping roles per theme. **Draw a divider only where two regions share a surface** (the column divider in `App.tsx` is the load-bearing one: editor content and diagram canvas are both page).

Dark mode needs stronger separations than daisyUI ships, for the reason `index.css`'s first comment gives: near black, a perceptually-normal step lands on very few sRGB code values. The light ramp is widened to match, so both themes put a band 13 code values off the page. Three consequences worth not undoing:
...
- **Anything relying on daisyUI's ghost-button hover is already handled in `index.css`.** Its hover is a hardcoded 7% mix toward black, so in dark it lands *darker* than the surface (measured RGB(17,21,27) on RGB(20,25,31)) whichever way the ramp runs. There's one override there covering every ghost button, so hover means one thing app-wide; don't re-solve it per component.
- **`modal-box` is overridden to the band surface in `index.css`.** daisyUI gives it `base-100`, which is now the page, so a modal painted the page colour onto the page — zero code values apart, with only a 25%-black shadow between them. A modal is raised chrome and takes a band like any other.
