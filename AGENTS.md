...
- **A type's colour is decided once, in the document's `StyleConfig`**, and everything that shows that type reads it from there: the diagram's `classDef`, the legend, and the editor, where a highlighted marker is drawn in its type's stroke (`index.css`'s `.tok-*` rules own how a `HighlightKind` is painted, and no ontology names a colour). Restyling a type in the **Style** dialog has to move all three together.
