...
- When an ontology's model doesn't fit a plain node-and-edge graph, parse into its own model and flatten in a separate `toGraph.ts`, rather than contorting the parser. `arg-map-truth-and-relevance` does this: a link there can be argued about like a claim, so `toGraph.ts` reifies links into nodes, and the rendering bet is swappable by rewriting that one file.
