...
- A *feature* is a switchable rendering lens an ontology declares and `components/FeatureStrip.tsx` renders generically; only the ontology's `toMermaid` gives an option meaning. Read feature state through `src/ontology/features.ts`'s accessors, never by indexing — a partial state from an old link has to resolve to defaults.
